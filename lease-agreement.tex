\documentclass[9pt, letterpaper, oneside]{extarticle}
...
\newcommand{\landlordname}{FIXME:landlord-name}
\newcommand{\tenantname}{FIXME:tenant-name}
\newcommand{\address}{FIXME:address}
\newcommand{\placedescription}{FIXME:description-of-premises}

\newcommand{\termstartdate}{FIXME:start-date}
\newcommand{\termenddate}{FIXME:end-date}

\newcommand{\rentamount}{FIXME:rent-amount}
\newcommand{\depositamount}{FIXME:deposit-amount}

\newcommand{\feeamount}{\$35}

\begin{document}

 \maketitle

 \numparagraph{\textbf{INTRODUCTION:}\hspace{2mm}} \textbf{\landlordname} (``Owner'') rents to \textbf{\tenantname} (``Tenant'') and Tenant agrees to rent \textbf{\address}, \placedescription (the ``Premises''). No other portion of the building (the ``Building''), where the Premises is located, is included for lease unless expressly provided for in this Residential Tenancy Agreement and Lease (the ``Agreement''). The Premises is provided furnished; the furniture inventory describes the furniture and appliances which are provided at the inception of the tenancy.

 \numparagraph{\textbf{TERM:}\hspace{2mm}} The term of this rental shall begin on \textbf{\termstartdate} and end on \textbf{\termenddate}, and thereafter shall be month-to-month on the same terms and conditions as stated herein, save any changes lawfully made until terminated.
